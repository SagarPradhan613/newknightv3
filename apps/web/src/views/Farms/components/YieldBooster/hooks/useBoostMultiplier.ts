...
import { useBCakeFarmBoosterContract, useMasterchefPancake } from 'hooks/useContract'
...
async function getMultiplierFromMC({
  ...
  masterChefContract: ReturnType<typeof useMasterchefPancake>
}): Promise<number> {
...
export default function useBoostMultiplier({ pid, boosterState, proxyAddress }): number {
  ...
  const masterChefContract = useMasterchefPancake()
