        ...
        <>
          {cakePoolLockExpired ? (
            <Message variant="warning" icon={<InfoFilledIcon color="warning" />}>
              <Text as="p">
                {t(
                  'Knight Pool migrated position has unlocked. Go to the pool page to withdraw, add Knight into veKnight to increase your veKnight benefits.',
                )}
              </Text>
            </Message>
          ) : (
            <Message variant="primary" icon={<InfoFilledIcon color="secondary" />}>
              <AutoColumn gap="8px">
                <Text as="p">
                  {t(
                    'Position migrated from Knight Pool can not be extended or topped up. To extend or add more Knight, set up a native veKnight position.',
                  )}
                </Text>
                <LearnMore />
              </AutoColumn>
      ...
      {cakeLockExpired && !cakePoolLockExpired ? (
        <Message variant="warning" icon={<InfoFilledIcon color={CUSTOM_WARNING_COLOR} />}>
          <Text as="p" color={CUSTOM_WARNING_COLOR}>
            {t(
              'Renew your veKNIGHT position to continue enjoying the benefits of weekly KNIGHT yield, revenue share, gauges voting, farm yield boosting, participating in IFOs, and so much more!',
            )}
          </Text>
        </Message>
      ) : null}
...
const SingleStakeTooltip = () => {
  ...
  return (
    <>
      {t('veKnight is calculated with number of KNIGHT locked, and the remaining time against maximum lock time.')}
      <LearnMore />
    </>
  )
}
...
const DualStakeTooltip: React.FC<{
  ...
  return (
    <>
      ...
      <ul>
        <li>
          {t('Native:')} {formatNumber(nativeBalance, 2, 4)} veKnight
        </li>
        <li>
          {t('Migrated:')} {formatNumber(proxyBalance, 2, 4)} veKnight
        </li>
      </ul>
      <br />
      <LearnMore />
...
const ProxyUnlockTooltip: React.FC<{
  ...
  return (
    <>
      {t(
        proxyExpired
          ? '%amount% KNIGHT from KNIGHT Pool migrated position is already unlocked. Go to the pool page to withdraw these KNIGHT.'
          : '%amount% KNIGHT from KNIGHT Pool migrated position will unlock on %expiredAt%.',
        {
          amount: proxyCake,
