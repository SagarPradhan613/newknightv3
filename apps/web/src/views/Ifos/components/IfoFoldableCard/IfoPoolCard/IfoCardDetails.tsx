...
const MaxTokenEntry = ({
  ...
  const basicTooltipContent =
    ifo.version >= 3.1 && !basicSale
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of KNIGHT up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max KNIGHT entry is capped by minimum between your average KNIGHT balance in the iKNIGHT, or the pool’s hard cap. To increase the max entry, Stake more KNIGHT into the iKNIGHT',
        )

  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      <Box>
        <Text display="inline">{t('For the public sale, Max KNIGHT entry is capped by')} </Text>
        <Text bold display="inline">
          {t('the number of iKNIGHT.')}{' '}
        </Text>
        <Text display="inline">
          {t(
            'Lock more KNIGHT for longer durations to increase the maximum number of CAKE you can commit to the sale.',
          )}
        </Text>
      </Box>
    ) : (
      t(
        'For the unlimited sale, Max Knight entry is capped by your average Knight balance in the iKnight. To increase the max entry, Stake more Knight into the iKnight',
      )
    )
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. Knight entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {raisingTokenToBurn && <FooterEntry label={t('KNIGHT to burn:')} value={raisingTokenToBurn} />}
          <FooterEntry
