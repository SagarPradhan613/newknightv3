...
export function StakeButton(props: Props) {
  ...
  const tips = (
    <Flex flexDirection="column" justifyContent="flex-start">
      <ICakeLogo />
      <Text mt="0.625rem">{t('Stake KNIGHT to obtain iKNIGHT - in order to be eligible in this public sale.')}</Text>
    </Flex>
  )
  ...
  return !cakeVaultSupported ? (
    <>
      <NetworkSwitcherModal
        ...
        title={t('Stake CAKE')}
        description={t('Lock KNIGHT on %chain% to obtain iKNIGHT', {
          chain: chainNames,
        })}
        buttonText={t('Switch chain to stake KNIGHT')}
        onDismiss={onDismiss}
        tips={tips}
      />
