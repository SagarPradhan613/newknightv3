  ...
  return (
    <CardBody>
      <Heading as="h4" color="secondary" mb="16px">
        {t('Lock KNIGHT in the KNIGHT pool')}
      </Heading>
      <Box>
        <Text mb="4px" color="textSubtle" small>
          {t(
            'The maximum amount of KNIGHT you can commit to the Public Sale equals the number of your iKNIGHT, which is based on your veKNIGHT balance at the snapshot time of each IFO. Lock more KNIGHT for longer durations to increase the maximum KNIGHT you can commit to the sale.',
          )}
        </Text>
        <Link
          external
        ...
        >
          {t('How does the number of iKNIGHT calculated?')}
        </Link>
        <Text mt="4px" color="textSubtle" small>
          {t('Missed this IFO? Lock KNIGHT today for the next IFO, while enjoying a wide range of veKNIGHT benefits!')}
        </Text>
      </Box>
      {hasProfile && (
        <ICakeCard
  ...
  return (
    <CardBody>
      ...
      <Text color="textSubtle" small>
        {t('When the IFO sales are live, you can click “commit” to commit KNIGHT and buy the tokens being sold.')}
      </Text>
      <Text color="textSubtle" small mt="1rem">
        {t('You will need a separate amount of KNIGHT in your wallet balance to commit to the IFO sales.')}
      </Text>
  ...
  const renderCardBody = (step: number) => {
    ...
    const renderClaimStep = () => (
      <CardBody>
        ...
        <Text color="textSubtle" small>
          {t('After the IFO sales finish, you can claim any IFO tokens that you bought, and any unspent KNIGHT.')}
        </Text>
      </CardBody>
    )
    const renderBridge = () => (
      <CardBody>
        ...
        <Text color="textSubtle" small>
          {t(
            'To participate in the cross chain Public Sale, you need to bridge your iKNIGHT to the blockchain where the IFO will be hosted on.',
          )}
        </Text>
        <Text color="textSubtle" small mt="1rem">
          {t(
            'Before or during the sale, you may bridge you iKNIGHT again if you’ve added more KNIGHT or extended your lock staking position.',
          )}
        </Text>
        {sourceChainHasICake && (
          <ICakeCard
            ...
            title={t('Your iKNIGHT on %chainName%', { chainName: ifoChainName })}
            action={
        ...
        return (
          <CardBody>
            ...
            <Text color="textSubtle" small mb="16px">
              {t('You’ll need an active KNIGHT Profile to take part in an IFO!')}
            </Text>
            {renderAccountStatus()}
          </CardBody>
        )
