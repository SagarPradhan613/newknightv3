      ...
      {Number.isFinite(lockedCakeBalance) && (
        <Box>
          <Text>
            {isBoostingExpired
              ? t(
                  'Your vKNIGHT boosting was expired at the snapshot block. Renew your fixed-term staking position to activate the boost for future voting proposals.',
                )
              : t(
                  'Voting power is calculated using the staking amount and remaining staking duration of the fixed-term KNIGHT staking position at the block.',
                )}
          </Text>
          <Text bold m="10px 0">
            {`${t('KNIGHT locked:')} ${formatNumber(lockedCakeBalance, 0, 2)}`}
          </Text>
  ...
  return (
    <ModalInner mb="0">
      <Text as="p" mb="24px" fontSize="14px" color="textSubtle">
        {t(
          'Your voting power is determined by the amount of KNIGHT you held and the remaining duration on the fixed-term staking position (if you have one) at the block detailed below. KNIGHT held in other places does not contribute to your voting power.',
        )}
      </Text>
                ...
                <Flex mt="10px" flexDirection="column" alignItems="flex-end">
                  ...
                  <Flex>
                    ...
                    <Text ml="4px" color={isBoostingExpired ? 'warning' : 'secondary'} fontSize="12px">
                      {isBoostingExpired ? t('Boosting Expired') : t('Boosted by vKNIGHT')}
                    </Text>
                  </Flex>
                </Flex>
              </FixedTermCardInner>
